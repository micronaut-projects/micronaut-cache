...
You must also configure `micronaut.cache.oracle.datasource` with the datasource name that will host the Oracle cache schema and runtime operations.
...
For example, if you setup a datasource called `mydatasource` with Micronaut Data, you can set it to host your caches with:
